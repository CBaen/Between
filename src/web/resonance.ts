...
export function setupResonance(server: Server): void {
  const wss = createPathServer('/resonance-ws');
...
<head>
  ...
  <style>
    ...
    * {
    ...
    /* Very subtle ambient - doesn't interfere with sound interaction */
    .ambient {
      position: fixed;
      inset: 0;
      pointer-events: none;
    ...
    .ambient-1 {
      ...
      top: -30%;
      left: -20%;
      animation: ambientDrift1 90s ease-in-out infinite;
    }
    ...
    @keyframes ambientDrift2 {
      0%, 100% { transform: translate(0, 0) scale(1); }
      50% { transform: translate(-10%, -6%) scale(0.92); }
    }
